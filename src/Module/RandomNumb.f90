!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
! PROGRAM: A subroutine containing several functions and subroutines to define several modern, fast highly reliable, 
!             ease-to-use, and state-of-the-art pseudo-random number generators, which are essential in massively
...
!! spring_sfmt_gaussian:  use polar form of the Box-Muller transformation to Generating Gaussian Random Numbers
!!                        ( with zero mean and standard deviation one ).
      function spring_sfmt_gaussian() result(r)
         implicit none
         real(ieee64) :: r
         real(ieee64) :: x1, x2, w, y1, y2
         w = 0.0_ieee64
         do while ( w .ge. 1.0_ieee64 )
            x1 = 2.0_ieee64 * spring_sfmt_string() - 1.0_ieee64
            x2 = 2.0_ieee64 * spring_sfmt_string() - 1.0_ieee64
            w = x1*x1 + x2*x2
         end do
         w = dsqrt( (-2.0_ieee64 * log(w) ) / w )
         r = x1 * w
      end function spring_sfmt_gaussian

   end module RandomNumb